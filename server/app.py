#[Depreceated/ Dead code]
import streamlit as st
import pandas as pd
import json

from pipelines.preprocessing_pipeline import run_pipeline

from agents.health_score_agent import generate_health_explanation

from utils.visualizations import plot_histograms, plot_correlation
...
if uploaded_file is not None:
    ...
    if st.button("Run Analysis"):
        ...
        st.write("### Quick Preprocessing Logs")
        st.write(result["logs"]["quick_preprocessing"])
        ...
        st.write("### Skewness")
        st.write(result["skewness"])

        st.metric("### Health Score", result["health_score"])
        report = {k: v for k, v in result.items() if k not in ["original_df", "processed_df"]}
        ...
        if result['corr'] is not None:
            ...
            fig = plot_correlation(result['corr'])

            st.pyplot(fig)
        

        st.write("### LLM Recommended Steps")
        st.write(result["steps"])

        st.write("### Applied Steps")
        st.write(result["logs"]["applied_steps"])

        st.write("### Processed Dataset")
        st.dataframe(result["processed_df"].head())


    # st.write('##-------------------------------------------')
    # st.write("### Dataset Preview are initial preprocessing")
    # st.dataframe(df.head())
